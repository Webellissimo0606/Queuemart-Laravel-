...
!function($) {
    ...
    CalendarApp.prototype.onEventClick =  function (calEvent, jsEvent, view) {
        var $this = this;
            ...
            $this.$modal.find('.delete-event').show().end().find('.save-event').hide().end().find('.modal-body').empty().prepend(form).end().find('.delete-event').unbind('click').click(function () {
                var d_date = new Date(calEvent.start);
                var holiday_date_d = d_date.toJSON().substring(0,10);

                $.get(ajax_delete_url, 
                    {
                        holiday_date: holiday_date_d,
                    }, function(data){
                    });
            ...
            $this.$modal.find('form').on('submit', function () {
                ...
                var u_date = new Date(calEvent.start);
                var holiday_date_u = u_date.toJSON().substring(0,10);

                $.post(ajax_update_url, 
                    {
                        holiday_date: holiday_date_u,
                        holiday_for: calEvent.title
                    }, function(data){
                    });
    ...
    CalendarApp.prototype.onSelect = function (start, end, allDay) {
        var $this = this;
            ...
            $this.$modal.find('form').on('submit', function () {
                ...
                var h_date = new Date(start);
                var holiday_date = h_date.toJSON().substring(0,10);
    ...
    CalendarApp.prototype.init = function() {
        ...
        var defaultEvents =  calendar_holiday;
